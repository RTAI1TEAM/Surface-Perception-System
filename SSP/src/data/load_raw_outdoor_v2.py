...
def featurize_window(win: pd.DataFrame) -> dict:
    ...
    # ── 속도 보정 수직 가속도 ────────────────────────────────────────────────
    # 포트홀 충격 크기는 속도에 비례하므로, 속도로 나눠 정규화
    ...
    # ── 3. 포트홀 충격 특이 피처 (수직 Z축) ─────────────────────────────────
    # 3-1. 충격 피크 감지 (scipy)
    peaks, props = find_peaks(np.abs(az), height=az.std(), distance=3)
    f["az_peak_count"]      = len(peaks)
    f["az_peak_height_max"] = props["peak_heights"].max() if len(peaks) > 0 else 0.0
 
    # 3-2. 충격 후 복원 패턴: 피크 이후 반전 진폭
    if len(peaks) > 0:
        p = peaks[0]
    ...
    # 3-4. 주파수 도메인 (포트홀 = 1~5Hz 충격)
    f["az_fft_dom_ratio"] = _dominant_freq_energy(az)
 
    # 3-5. 변화율
    diff_az = np.diff(az)
    f["az_diff_std"]  = diff_az.std()
    f["az_diff_max"]  = diff_az.max()
    f["az_diff_min"]  = diff_az.min()
 
    # ── 4. 교차 축 피처 ────────────────────────────────────────────────────────
    # 포트홀은 Z 충격이 크고 X·Y는 상대적으로 작아야 함
...
def build_dataset(base_path: str, save_path: str):
    ...
    full_df.to_csv(os.path.join(save_path, "full_v2_r5.csv"), index=False)
 
    # (b) 기존 호환: 마지막 trip을 test로 고정한 단일 split
    last_split = splits[-1]
    last_split["X_train"].assign(label=last_split["y_train"]).to_csv(
        os.path.join(save_path, "train_v2_r5.csv"), index=False
    )
    last_split["X_test"].assign(label=last_split["y_test"]).to_csv(
        os.path.join(save_path, "test_v2_r5.csv"), index=False
    )

# XGBoost Base model (Auto Save Version)
...
# =====================
DATASET_NAME = "v3_s2"
MODEL_NAME = f"xgb_base_{DATASET_NAME}_model"
THRESHOLD = 0.5
# ======================
# 저장 폴더 자동 생성
# ======================
...
# 시각화 함수
# ======================
def visualization(model, x_test, y_test, save_dir, threshold=0.5):
...
TRAIN_PATH = os.path.join(BASE_PATH, f"train_{DATASET_NAME}.csv")
TEST_PATH = os.path.join(BASE_PATH, f"test_{DATASET_NAME}.csv")
...
# 모델 저장 (joblib)
# ======================
model_path = os.path.join(project_root, "models", "pothole", f"{MODEL_NAME}.pkl")
...
plt.barh(xgb_gain_pct.head(10).index[::-1], xgb_gain_pct.head(10).values[::-1])

...
수집된 원본(Raw) 데이터의 통계적 특성 및 구조적 문제를 파악하기 위해 전처리 이전 데이터를 대상으로 탐색적 분석을 수행하였다. 각 시각화 자료에서 도출된 발견 사항은 이후 전처리 기법 선정의 근거로 활용된다.

3.1 실내 데이터 (Indoor) 원본 분석 결과

1) 바닥 재질별 원본 시계열 파형 비교

[그림 3-1] 바닥 재질별 원본 선형 가속도 Z축 시계열 파형 (전처리 전)
<img src="./SSP/reports/figures/indoor_raw/01_raw_timeseries_per_surface.png" width="500">

각 바닥 재질에서 대표 시퀀스(128 Time-step)의 선형 가속도 Z축 원시 파형을 비교한 결과, 재질 간 진폭의 차이가 존재하나 파형 자체만으로는 재질을 명확히 구분하기 어렵다. 이는 단순한 순간 센서 값이 아닌, 시퀀스 전체의 통계적 분포(표준편차, 왜도, 첨도 등)를 피처로 추출해야 함을 시사한다.

2) 원본 쿼터니언 축 간 다중공선성 확인

[그림 3-2] 원본 쿼터니언 (Orientation) 축 간 상관관계 및 산점도 (전처리 전)
<img src="./SSP/reports/figures/indoor_raw/02_raw_quaternion_corr.png" width="500">

원본 쿼터니언의 orientation_X와 orientation_W 축 간 상관계수가 r=1.00으로, 완전한 선형 의존 관계(완전 다중공선성)가 존재함이 확인되었다. 이러한 변수들을 그대로 모델 입력으로 사용할 경우 중복 정보로 인해 모델 학습의 효율과 안정성이 저하된다. 이를 해소하기 위해 4차원 쿼터니언을 3개의 독립적인 오일러 각도(Roll, Pitch, Yaw)로 변환하는 전처리가 필수적이다.

3) 원본 가속도 값의 클래스 간 분포 비교

[그림 3-3] 바닥 재질별 원본 선형 가속도 Z축 값 분포 비교 (전처리 전)
<img src="./SSP/reports/figures/indoor_raw/03_raw_accel_distribution.png" width="500">

전체 재질의 원시 linear_acceleration_Z 값 분포가 동일한 구간에 완전히 중첩되어 있음이 확인되었다. 이는 원시 센서 값의 단순 평균만으로는 재질 간 분류가 불가능함을 의미하며, 진폭 크기(Magnitude), 변화율(Diff), 고차 통계량(왜도·첨도) 등 복합적인 피처 엔지니어링이 반드시 수행되어야 함을 뒷받침한다.
...
4.3 전처리 결과 검증 (Post-Preprocessing Validation)

전처리 파이프라인 적용 후 생성된 피처 데이터셋을 시각화하여, 원본 데이터 대비 클래스 분리도가 향상되었는지 검증하였다.

1) 전처리 후 실내 피처 분포 (재질별 박스플롯)

[그림 4-1] 전처리 후 주요 파생 피처별 바닥 재질 분포 비교
<img src="./SSP/reports/figures/indoor_eda/02_feature_distributions.png" width="500">

전처리 전에는 원시 가속도 Z축 값의 분포가 모든 재질에서 완전히 중첩되어 있었으나 (그림 3-3 참고), 피처 엔지니어링 적용 후 가속도 크기(accel_mag_mean), 각속도 크기(gyro_mag_mean) 등의 파생 변수에서 재질 간 분포 차이가 명확하게 드러남을 확인하였다. 이는 전처리를 통해 클래스 판별에 유의미한 피처가 성공적으로 추출되었음을 의미한다.

2) 전처리 후 피처 공간의 클래스 분리도 (PCA)

[그림 4-2] 전처리된 피처의 PCA 2차원 투영 산점도
<img src="./SSP/reports/figures/indoor_eda/04_pca_scatter.png" width="500">

전처리 후 피처를 PCA로 2차원 투영한 결과, 일부 재질 클래스(예: hard_tiles_large_space)가 타 클래스로부터 어느 정도 분리되는 군집 구조를 보이며, 전처리 전보다 클래스 간 구분 가능성이 향상되었음을 확인하였다. 다만 다수의 클래스가 여전히 비선형적으로 중첩되어 있어, 트리 기반 앙상블 모델(Random Forest, XGBoost)의 적용이 적합하다는 판단을 재확인하였다.
